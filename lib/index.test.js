...
var sattrack = require('./index.js');
var moment = require('moment-timezone');
var SGP4 = require('sgp4');
var sexa = require('astronomia').sexagesimal;
...
`;
  ...
  function getGmst(date) {
    return SGP4.gstimeFromDate(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds());
  }
  
  function near(a, b) {
    // console.log('near', a, b, Math.abs(a - b));
    return Math.abs(a - b) < 0.03;
  }
  
  it('should convert alt/az to ra/dec', function() {
    var rtd = 57.29577951;
    var lat = sexa.DMSToDeg(false, 34, 38, 1.2);
    var lon = sexa.DMSToDeg(true, 86, 32, 13);
    var t = new Date(2017, 3, 14, 20);
    var gmst = getGmst(t);
    // console.log('convert', lat, lon, gmst, t.toLocaleString());
    var e = sattrack.AltAz2RaDec(gmst * rtd, 31.103300, 214.420825, lat, lon);
    // console.log('e', e, new sexa.RA(e.ra), new sexa.RA(6, 45, 54.78));
    assert(near(e.ra, 6.765217 * 15));
    assert(near(e.dec, -16.737009));
  });
  
  it('should convert alt/az to ra/dec in degrees', function() {
    var lat = sexa.DMSToDeg(false, 34, 38, 1.2);
    var lon = sexa.DMSToDeg(true, 86, 32, 13);
    var t = new Date(2017, 3, 14, 20);
    var gmst = getGmst(t);
    // console.log('convert', lat, lon, gmst, t.toLocaleString());
    var e = sattrack.AltAz2RaDecDeg(gmst, 31.103300, 214.420825, lat, lon);
    // console.log('e', e, new sexa.RA(e.ra), new sexa.RA(6, 45, 54.78));
    assert(near(e.ra, 6.765217 * 15));
    assert(near(e.dec, -16.737009));
  });
